...
public class FontRenderer {
	...
	public FontRenderer(String fontPath) throws Exception {
		...
		fontBuffer.rewind();
		if (stbtt_BakeFontBitmap(fontBuffer, FONT_SIZE, bitmap, FONT_BITMAP_WIDTH, FONT_BITMAP_HEIGHT, ASCII_START, charData) <= 0) {
	...
	public float getWidth(String text, float scale) {
		...
		try (MemoryStack stack = MemoryStack.stackPush()) {
			...
			for (char c : text.toCharArray()) {
				...
				stbtt_GetBakedQuad(charData, FONT_BITMAP_WIDTH, FONT_BITMAP_HEIGHT,
						c - ASCII_START, xBuf, yBuf, q, true);
			}
			x = xBuf.get(0);
		}
	...
	public void renderText(String text, float x, float y, float scale, Color col) {
		y+=FONT_SIZE;
		...
		// Set projection
		float[] projection = Matrix4f.ortho2D(0, win.w, win.h, 0);
		...
		int colorLoc = glGetUniformLocation(shader, "textColor");
		float[] f = col.getRGBComponents(null);
		glUniform4f(colorLoc, f[0], f[1], f[2], f[3]);
	...
	public void renderText(String text, float x, float y, float scale) {
		renderText(text, x, y, scale, Color.white);
	}
	...
	private int compileShader(String vertexPath, String fragmentPath) throws IOException {
		...
		return new Shader(frag, vert).shaderId;
	}
}